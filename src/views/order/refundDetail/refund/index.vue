  ...
  <div class="order-detail-refund">
    ...
    <div class="order-detail-content">
      <refund-detail :data="info" v-if="info.refund_info">
        <refund-modal :visible="modal.refundModal" :data="info"></refund-modal>
      </refund-detail>
    </div>
    <div class="order-detail-con">
        ...
        <distributor-information
            ...
            @edit="editCommision"
        ></distributor-information>
        <order-detail-list is-refund :data="info" @on-change-goods-info="changeGoodsInfo"
                           v-if="clickIndex==0"></order-detail-list>
        <commission-edit :orderId="id" :orderNo="orderNo" :memberId="member_id" :visible.sync="visible"
                         @on-success="getData"></commission-edit>
    </div>
  </div>
</template>
...
export default {
  ...
  watch: {
    status(val) {
      if (val === 0) {
        this.step.lastTitle = '退款完成'
      } else if (val === 10 || val === 11) {
        this.step.lastTitle = '退款成功'
      } else {
        this.step.lastTitle = '退款失败'
      }
    }
  },
  data() {
