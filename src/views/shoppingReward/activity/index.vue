...
export default {
    ...
    methods: {
        ...
        getShoppingRewardList() {
            ...
            this.$api.shoppingRewardApi.getShoppingRewardList(params).then((res) => {
                this.loading = false
                if (res.error === 0) {
                    this.data = res.list
        ...
        handleStop(id) {
            this.$Modal.confirm({
                ...
                onOk: () => {
                    this.stopShoppingRewardActivity(id)
                },
                onCancel: () => {},
            })
        },
        // 停止活动
        stopShoppingRewardActivity(id) {
        ...
        handleDelete(id) {
            this.$Modal.confirm({
                ...
                content: '确定要删除该活动？',
                onOk: () => {
                    this.deleteShoppingRewardActivity(id)
                },
                onCancel: () => {},
            })
        },
        // 删除活动
        deleteShoppingRewardActivity(id) {
            this.$api.shoppingRewardApi
