...
<template>
    <kdx-header-bar>
        <template #header>
            <Button type="primary" @click="addActivity" :disabled="noManagePerm">+添加活动</Button>
        </template>
        <Form ref="form" :model="model" :label-width="100" inline @submit.native.prevent>
            <FormItem label="活动名称：">
                <i-input type="text" v-model="model.activity_name" placeholder="活动名称" @on-enter="handleSearch" class="width-340"/>
            </FormItem>
            <FormItem label="活动状态：">
            ...
            <FormItem label="活动时间：">
                <DatePicker v-model="date" class="width-340" type="datetimerange" format="yyyy-MM-dd HH:mm" placeholder="活动时间" :editable="false" @on-change="changeDate"></DatePicker>
            </FormItem>
...
<script>
    export default {
        ...
        methods: {
            ...
            //         .then(res => {
            //             if (res.error === 0) {
            //                 this.level = this.level.concat(res.list);
            //             }
            //         })
            // },
            // 添加活动
