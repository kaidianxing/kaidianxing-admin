        ...
        <div class="table-list" v-loading="loading">
            <Table ref="table" :columns="columns" :data="data">
                ...
                <template slot-scope="{ row }" slot="reward">
                    <div class="gifts" v-if="row.reward">
                        <div
                            class="gifts-coupon mb"
                            v-if="
                        ...
                        <div class="gifts-credit mb" v-if="row.reward.red_package">
                            红包：￥{{ row.reward.red_package.money }}
                        </div>
                        <div class="gifts-credit mb" v-if="row.reward.credit">
                            积分：{{ row.reward.credit }}
                        </div>
                        <div class="gifts-balance mb" v-if="row.reward.balance">
                            余额：￥{{ parseFloat(row.reward.balance) }}
                        </div>
                    </div>
                </template>
            </Table>
...
export default {
                    ...
                    render: (h, params) => {
                        return (
                            <Button
                                type="text"
                                onClick={() => {
                                    this.toOrderDetail(params.row.order_no)
                                }}
                            >
                                {params.row.order_no}
                            </Button>
    ...
    methods: {
        ...
        getConsumeLog() {
            ...
            this.$api.consumeRewardApi.getConsumeLog(params).then((res) => {
                this.loading = false
                if (res.error === 0) {
                    this.data = res.list
        ...
        toOrderDetail(orderNo) {
            ...
            this.$utils.openNewWindowPage(`/order/list/all`, query)
        },
    },
}
</script>
