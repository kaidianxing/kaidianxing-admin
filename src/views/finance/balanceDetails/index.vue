...
export default {
                        ...
                        if (parseFloat(params.row.num) > 0) {
                            ...
                            return (
                                <kdx-status-text type="danger" class="num-change">
                                    -￥{params.row.num.slice(1)}
                                </kdx-status-text>
                            )
                        } else {
                            return <div>{params.row.num}</div>
                        }
                    },
                },
                {
    ...
    methods: {
        ...
        getBalanceList() {
            ...
            this.loading = true
            this.$api.financeApi.getBalanceList(_params).then((res) => {
                this.loading = false
                if (res.error == 0) {
        ...
        handleExport() {
            ...
            // let downloadurl = `${base_url}manage/finance/credit-record/balance&level_id=${level_id}&keyword=${keyword}&page=${page}&pagesize=${pagesize}&export=1`
            if (this.searchData.startTime) {
                // downloadurl = `${downloadurl}&created_at[0]=${this.searchData.startTime}`
                params.crete_time[0] = this.searchData.startTime;
            }
            if (this.searchData.endTime) {
            ...
            Export('manage/finance/credit-record/balance', params);
        },
        // 切换页码
        changePage(page) {
