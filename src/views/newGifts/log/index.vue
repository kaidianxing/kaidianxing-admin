    ...
    <div class="activity-wrap">
        ...
        <div class="table-list" v-loading="loading">
            <Table ref="table" :columns="columns" :data="data">
                ...
                <template slot-scope="{ row }" slot="gifts">
                    <div class="gifts" v-if="row.gifts">
                        <div
                            class="gifts-coupon mb"
                            v-if="
                                row.gifts.coupon_title &&
                                    row.gifts.coupon_title.length
                            "
                        >
                            <div
                                class="coupon-item mb"
                                v-for="(item, index) in row.gifts.coupon_title"
                                :key="index"
                            >
                                {{ item }}
                            </div>
                        </div>
                        <div class="gifts-credit mb" v-if="row.gifts.credit">
                            积分：{{ row.gifts.credit }}
                        </div>
                        <div class="gifts-balance mb" v-if="row.gifts.balance">
                            余额：￥{{ parseFloat(row.gifts.balance) }}
                        </div>
                    </div>
                </template>
            </Table>
...
export default {
    ...
    methods: {
        ...
        getNewGiftsLog() {
            ...
            this.$api.newGiftsApi.getNewGiftsLog(params).then((res) => {
                this.loading = false
                if (res.error === 0) {
                    this.data = res.list
