    ...
    <div class="level">
        ...
        <FormItem label="规格设置：" prop="has_option" class="r-form-item-checkbox">
            <RadioGroup class="flex" v-model="model_has_option">
                ...
                <Radio label="1" :disabled="is_activity_goods">
                    <span>多规格</span>
                </Radio>
                <div class="flex">
            ...
            <kdx-hint-alert style="margin-top: 10px">如切换规格后，会员折扣需要重新设置，如果不设置则为系统默认该商品不支持会员折扣</kdx-hint-alert>

            <!--单规格和多规格切换-->
            <div class="nest-box" style="padding: 0">
                <!--单规格-->
                <div class="single-one" style="padding-top:30px;" v-if="model_has_option === '0'">
                    <FormItem label="售卖价：" prop="price"  v-error-item.price>
                ...
                <div style="padding-bottom:30px;" v-if="model_has_option === '0'">
                    <FormItem label="库存：" prop="stock" style="margin-bottom: 0;display: flex" v-error-item.stock v-if="model_type != '2'">
                    ...
                    <FormItem label="卡密库：" prop="virtual_account_id" style="margin-bottom: 0;display: flex" v-error-item.virtual_account_id v-else>
                        <Select style="width:520px"  filterable
                                clearable v-model="model_virtual_account_id" not-found-text='暂无卡密库数据'>
                ...
                <div class="single-more" v-else-if="model_has_option === '1'">
                    <multi-option @form-error="formError" :model_type="model_type" :actionType ='actionType' :flag="1"></multi-option>
                </div>
            </div>
        </FormItem>
