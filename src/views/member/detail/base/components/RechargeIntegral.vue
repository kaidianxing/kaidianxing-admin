 ...
 */
<template>
    <kdx-modal-frame v-model="value" title="积分充值" :width="610" @on-cancel="handleCancel">
        <div class="recharge-integral-modal">
            <Form ref="form" :label-width="120" :model="model" :rules="rules"
                  @submit.native.prevent>
                <FormItem label="当前积分：">
                    <div class="integral-data">{{data}}</div>
                </FormItem>
                <FormItem label="变化：" class="r-form-item-checkbox">
                    <RadioGroup v-model="model.change_type">
                        ...
                        <Radio label="0">最终积分</Radio>
                    </RadioGroup>
                </FormItem>
                <FormItem label="充值数目：" prop="num">
                    <kdx-rr-input v-model="model.num"  :fixed="0" :max-value="9999999" number class="width-160" placeholder="请输入充值数目">
        ...
        methods: {
            ...
            handleOk() {
                ...
                this.$refs['form'].validate(valid => {
                    if (valid) {
                        let params = Object.assign({id: this.id, type: 'credit'}, this.model);
                        this.loading = true
