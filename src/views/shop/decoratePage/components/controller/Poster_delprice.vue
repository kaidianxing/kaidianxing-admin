 ...
 */
<template>
    <div class='poster_delprice' v-if='currentModal.id=="poster_delprice"&&lazyLoad'>
        <i-form :label-width="100" ref='pictures' :model="currentModal" :rules="validator">
            <MyCollapse no-border>
                <p class="form-title" slot='label'>
                    内容样式
                </p>
                <form-item label="商品原价：" style='margin-bottom:10px;'>
                    <Input placeholder="请输入商品原价" v-model='currentModal.params.content' style="width: 160px;">
                    </Input>
                    <Alert style='margin-top:10px;'>提示：此数据只作为展示使用，真实数据以获取的商品原价为准。</Alert>
                </form-item>
