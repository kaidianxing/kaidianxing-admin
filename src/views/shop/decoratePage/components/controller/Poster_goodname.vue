 ...
 */
<template>
    <div class='poster_goodname' v-if='currentModal.id=="poster_goodname"&&lazyLoad'>
        <i-form :label-width="100" ref='pictures' :model="currentModal" :rules="validator">
            <MyCollapse no-border>
                <p class="form-title" slot='label'>
                    内容样式
                </p>
                <form-item label="商品名称：" style='margin-bottom:10px;'>
                    <Input placeholder="请输入商品名" v-model='currentModal.params.content' style="width: 400px;">
                    </Input>
                    <Alert style='margin-top:10px;'>提示：此数据只作为展示使用，真实数据以获取的商品名称为准。</Alert>
                </form-item>
