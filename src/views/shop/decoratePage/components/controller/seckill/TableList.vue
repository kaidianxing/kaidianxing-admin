...
<template>
    <div class="select-good1">
        ...
        <div class="add" @click="addGood" v-if="componentData.data && componentData.data.length < 20">+添加商品({{componentData.data.length}}/20)</div>
        <!--商品选择器-->
        <goods-selector
            v-model="show"
            multiple
...
export default {
    ...
    methods: {
        ...
        handleChange(val) {
            ...
            let len = val.length<=20? val.length:20;
            for (let i = 0; i < len; i++) {
                let item = val[i]
                this.componentData.data.push({
                    ...
                    sales: item.sales * 1 + item.real_sales * 1,
