 ...
 */
<template>
    <div class='poster_price' v-if='currentModal.id=="poster_price"&&lazyLoad'>
          <i-form :label-width="100" ref='pictures' :model="currentModal" :rules="validator">
            <MyCollapse no-border>
                <p class="form-title" slot='label'>
                    内容样式
                </p>
                <form-item label="商品会员价：" style='margin-bottom:10px;'>
                    <kdx-rr-input number :fixed="2" :maxValue="9999999.99" placeholder="请输入商品会员价" v-model='currentModal.params.content' style="width: 160px;">
                    </kdx-rr-input>
                    <Alert style='margin-top:10px;'>提示：此数据只作为展示使用，真实数据以获取的商品现价为准。</Alert>
                </form-item>
                ...
                <form-item label="字体颜色：" style='margin-top:10px;'>
                    <MyColorPicker v-model='currentModal.style.color' default='#FF3C29'></MyColorPicker>
                </form-item>
