 ...
 */
<template>
    <div class='poster_exptime' v-if='currentModal.id=="poster_exptime"&&lazyLoad'>
         <i-form :label-width="120" ref='pictures' :model="currentModal" :rules="validator">
            <MyCollapse no-border>
                <p class="form-title" slot='label'>
                    内容样式
                </p>
                <form-item label="选择失效时间：" required>
                    <DatePicker type="date" format="yyyy-MM-dd" placeholder="选择日期" style="width: 200px" @on-change='getContent' :value='currentModal.params.content'></DatePicker>
                </form-item>
                ...
                <form-item label="字体颜色：" style='margin-top:10px;'>
                    <MyColorPicker v-model='currentModal.style.color' default='#969696'></MyColorPicker>
                </form-item>
    ...
    export default {
        ...
        methods: {
            getContent(date){
                ...
                this.currentModal.params.content=`${year}-${month>9?month:'0'+month}-${day>9?day:'0'+day}`
            },
            toCenter(val) {
                if (val == 'left') {
