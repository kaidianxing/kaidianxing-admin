 ...
 */
<template>
    <div class='poster_text' v-if='currentModal.id=="poster_text"&&lazyLoad'>
        <i-form :label-width="100" ref='poster_text' :model="currentModal" :rules="validator">
            <MyCollapse no-border>
                <p class="form-title" slot='label'>
                    内容样式
                </p>
                <form-item label="文本文字：" required prop='content'>
                    <Input placeholder="请输入文字" v-model='currentModal.params.content' @on-change='tipError'>
                    </Input>
                </form-item>
