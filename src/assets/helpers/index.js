...
export function getPreviewUrl(type, id, url) {
    ...
    if (type === 'goods') { // 商品详情
        url = `${url}/kdxGoods/detail/index?goods_id=${id}`;
    } else if (type === 'couponLink') { // 优惠券
        url = `${url}/kdxMember/coupon/detail/index?id=${id}`;
...
export function tomedia(src, type) {
    ...
    if (src.substr(0, 1) === '/') {
        if (src.indexOf(`/static/dist/shop/wap/`) == 0) {
            return src
        }
...
export function replaceImage(e, type) {
    ...
    switch (type) {
        ...
        case 'merchant-logo': // 店铺logo
            ...
            break;
        case 'shop': // 店铺logo
            path = require('@/assets/image/replace/shop.png')
            break;
        default:
            // 商品图片
