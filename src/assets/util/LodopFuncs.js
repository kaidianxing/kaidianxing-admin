...
export function getLodop(oOBJECT, oEMBED, printSetting = null) {
    // var strHtmInstall =
    ...
    try {
        ...
        //===如下空白位置适合调用统一功能(如注册语句、语言选择等):==
            console.error('--------------------1-------set success',printSetting)
        if(printSetting) {
            LODOP.SET_LICENSES("",printSetting.key,"","");
            console.error('----------------------2-----set success')
        }
        //=======================================================
        return LODOP
    } catch (err) {
        console.error('---------lodop Err',err)
        Modal.confirm({
            title: '提示',
            content:
